...
class IncidentModel(Base, TimestampMixin):
    ...
    candidate_causal_changes: Mapped[list[dict[str, Any]]] = mapped_column(
        JSON, nullable=False, default=list
    )
    causal_changes: Mapped[list[dict[str, Any]]] = mapped_column(
        JSON, nullable=False, default=list
    )  # Strictly verified causal changes only
